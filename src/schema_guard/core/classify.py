def classify_schema_diff(diff:dict, old_schema:dict, new_schema: dict)-> dict:
    ...
    for field in diff["nullable_changed"]:
        if field["from"] is True and field["to"] is False:
            breaking.append(f"Nullable changed for {field["field"]} from {field["from"]} to {field["to"]}")
        else:
            warnings.append(f"Nullable changed for {field["field"]} from {field["from"]} to {field["to"]}")
